/**
 * noscript.js - Feature Detection & IE Blocker for Network Speed Test
 */
(function() {
    'use strict';
    ...
    if (isInternetExplorer) {
        alert("Your Internet Explorer does not support Network Speed Test");
            ...
            container.innerHTML =
                '<div style="max-width: 600px; margin: 100px auto; padding: 40px; background: #1e293b; border: 2px solid #38bdf8; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">' +
                    ...
                    '<p style="font-size: 16px; line-height: 1.6; color: #cbd5e1; margin-top: 20px;">' +
                        'The Network Speed Test relies on modern web APIs including HTML5 Canvas, fetch API, fetch stream readers, and ES6+ JavaScript, which are completely incompatible with Internet Explorer.' +
                    '</p>' +
        ...
        throw new Error('Your Internet Explorer does not support Network Speed Test');
    }
    ...
    if (!hasFetch || !hasPromise) {
        console.warn("⚠️ browser-unsupported: Missing Fetch API or Promises. The Speed Test may not function correctly.");
    }
})();
